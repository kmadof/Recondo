...
CoursesUI::CoursesUI(RecondoModel* model, QWidget *parent, Qt::WFlags flags)
	: QDialog(parent, flags), _model(model)
{
	setupUi(this);
	...
	QObject::connect(btnEdit, SIGNAL(clicked()), this, SLOT(editCourse()));
...
void CoursesUI::editCourse()
{
	_presenter->GetCourseToEdit();
	cEdit = new CourseEditUI(_model);
	int result = cEdit->exec();
